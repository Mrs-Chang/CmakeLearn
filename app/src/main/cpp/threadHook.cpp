...
void enableThreadHook() {
    if (thread_hooked) {
        ALOG("GaoChang: return");
        return;
    }
    ALOG("GaoChang: enableThreadHook");
...
Java_com_chang_cmakelearn_ThreadHook_enableThreadHookNative(JNIEnv *env, jclass type) {
...
static bool InitJniEnv(JavaVM *vm) {
    ...
    kJavaClass = reinterpret_cast<jclass>(env->NewGlobalRef(env->FindClass("com/chang/cmakelearn/ThreadHook")));
    if (kJavaClass == NULL)  {
